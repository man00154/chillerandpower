...
menu = st.sidebar.radio(
    ...
    index=0,
    key="main_nav_menu",  # avoid duplicate element id error
)
...
if menu == "Chillers":
    ...
    NUM_CHILLERS = len(chillers)  # should be 30 in your JSON
    CHILLERS_PER_ROW = 10
    ...
    for row in range(num_rows):
        ...
        for col_idx in range(CHILLERS_PER_ROW):
            ...
            # Status bar
            color = "#00aa00" if ch.get("status", "OFF") == "ON" else "#aa0000"
            col.markdown(
                f"<div style='background:{color};color:white;text-align:center;"
                f"padding:4px;font-size:12px;'>STATUS: {ch.get('status','OFF')}</div>",
                unsafe_allow_html=True,
            )
            ...
            # Toggle button
            if col.button(f"Toggle {name}", key=f"ch_toggle_{idx}"):
...
# -------------------------------------------------------------
    ...
    for i, tr in enumerate(tr_list):
        if tr_cols[i].button(f"Toggle {tr['name']}", key=f"tr_btn_{i}"):
    ...
    for i, u in enumerate(ups_list):
        if ups_cols[i].button(f"Toggle {u['name']}", key=f"ups_btn_{i}"):
            toggle_ups(power, i)
            save_power(power)
            st.rerun()
    ...
    for i, g in enumerate(g_list):
        if g_cols[i].button(f"Toggle {g['name']}", key=f"gen_btn_{i}"):
    ...
    for i, p in enumerate(p_list):
        if p_cols[i].button(f"Toggle {p['name']}", key=f"pahu_btn_{i}"):
            toggle_pahu(power, i)
            save_power(power)
            st.rerun()
...
# -------------------------------------------------------------
    ...
    audio_file = st.file_uploader(
        "Upload WAV file (16-bit PCM recommended)", type=["wav"]
    )
